...
namespace astar_algorithms
{
    ...
    public:
        ...
        // Constructor that initializes Dijkstra's algorithm with a map, start, and goal positions
        Dijkstra(
            ...
            : map_(map), start_(start), goal_(goal), state_(AlgorithmState::NOT_STARTED)
        {
            ...
            distance_ = std::vector<std::vector<double>>(
        ...
        void initializeSearch() override
        {
            ...
            distance_[start_.first][start_.second] = 0;
            open_set_.push(std::make_pair(0.0, start_));
        ...
        bool stepSearch() override
        {
            ...
            for (const auto &dir : directions)
            {
                ...
                double tentative_distance = distance_[current_node_.first][current_node_.second] + 1;

                // If this path is better than previous one
                if (tentative_distance < distance_[next_x][next_y])
                {
                    came_from_[next_x][next_y] = current_node_;
                    distance_[next_x][next_y] = tentative_distance;
                    open_set_.push(std::make_pair(tentative_distance, neighbor));
        ...
        // Run complete Dijkstra's algorithm
    ...
    private:
        ...
        std::vector<std::vector<double>> distance_; // Distance from start
