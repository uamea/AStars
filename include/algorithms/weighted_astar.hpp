...
{
    class WeightedAStar : public IAStar
    {
    public:
        WeightedAStar() = default;
        WeightedAStar(const WeightedAStar &) = delete;
        WeightedAStar &operator=(const WeightedAStar &) = delete;

        // Constructor that initializes the Weighted A* algorithm with a map, start, goal positions, and weight
        WeightedAStar(
            const std::vector<std::vector<double>> &map,
            const std::pair<int, int> &start,
            const std::pair<int, int> &goal,
            double weight = 1.3) // Weight parameter
            : map_(map), start_(start), goal_(goal), weight_(weight), state_(AlgorithmState::NOT_STARTED)
        {
        ...
        void initializeSearch() override
        {
            ...
            double h = heuristic(start_, goal_);
            double f_score = g_score_[start_.first][start_.second] + weight_ * h;
        ...
        // Execute one step of the Weighted A* algorithm
        bool stepSearch() override
        {
            ...
            for (const auto &dir : directions)
            {
                ...
                if (tentative_g_score < g_score_[next_x][next_y])
                {
                    ...
                    // Apply weight to the heuristic component of f-score
                    double h = heuristic(neighbor, goal_);
                    double f = tentative_g_score + weight_ * h;
        ...
        // Run complete Weighted A* algorithm
    ...
    private:
        ...
        double weight_; // Weight factor for heuristic
